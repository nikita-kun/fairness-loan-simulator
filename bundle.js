function readParam(name, defaultValue){
	...
	var param = a.searchParams.get(name);
	return parseFloat(param ? param : defaultValue);
}
...
function main() {  
  ...
  var b0 = readParam("b0",1.4);
  var b1 = readParam("b1",1.2);
  
  var n0 = readParam("n0",1000);
  var n1 = readParam("n1",1000);
  var tprValue = readParam("tprValue",3);
  var fprValue = readParam("fprValue",-7);
  var render = readParam("render",1);
  
  document.getElementById("a0").value = a0
  document.getElementById("a1").value = a1
  document.getElementById("b0").value = b0
  document.getElementById("b1").value = b1
  
  document.getElementById("n0").value = n0
  document.getElementById("n1").value = n1
  document.getElementById("tprValue").value = tprValue
  document.getElementById("fprValue").value = fprValue
  document.getElementById("render").value = render
  
  comparisonExample0 = new GroupModel(makeBetaItems(0,n0,a0,b0), tprValue, fprValue);
  comparisonExample1 = new GroupModel(makeBetaItems(1,n1,a1,b1), tprValue, fprValue);
  ...
  document.getElementById('startBatch').onclick = startBatch;
  ...
  function debugOptimizer(optimizer){
  
  	optimizer();
  	output = { 
  		optimizer: optimizer.name,
  		profit: comparisonExample0.profit + comparisonExample1.profit,
		discriminationRate: ((countMatches(comparisonExample1.items, 1, 0) + 
			countMatches(comparisonExample0.items, 1, 0)) / (countMatches(comparisonExample1.items, 1, 0) + 
			countMatches(comparisonExample0.items, 1, 0) + countMatches(comparisonExample1.items, 1, 1) + 
			countMatches(comparisonExample0.items, 1, 1))).toFixed(3),
		discriminations: (countMatches(comparisonExample1.items, 1, 0) + 
			countMatches(comparisonExample0.items, 1, 0)),
		population: (comparisonExample1.items.length + comparisonExample0.items.length),
		groups: [
					{
						beta: [a0, b0],
						size: n0,
						profit: comparisonExample0.profit,
						threshold: comparisonExample0.threshold,
						positiveRate: comparisonExample0.positiveRate.toFixed(3),
						truePositiveRate: comparisonExample0.tpr.toFixed(3)
					},	
					{
						beta: [a1, b1],
						size: n1,
						profit: comparisonExample1.profit,
						threshold: comparisonExample1.threshold,
						positiveRate: comparisonExample1.positiveRate.toFixed(3),
						truePositiveRate: comparisonExample1.tpr.toFixed(3)
					}
				]
  	}
  
  	console.log(output);
  	document.getElementById("batch").innerHTML += JSON.stringify(output, null, 2) + "<br>";
  	return output;
  }

  function startBatch(){
  	  debugOptimizer(optimizer.maximizeProfit);
  	  debugOptimizer(optimizer.groupUnaware);
  	  debugOptimizer(optimizer.demographicParity);
  	  debugOptimizer(optimizer.equalOpportunity);
  }
  ...
  function updateTextDisplays(event) {
    ...
    display('total-discrimination', countMatches(comparisonExample1.items, 1, 0) + countMatches(comparisonExample0.items, 1, 0));
    display('total-discrimination-rate', ((countMatches(comparisonExample1.items, 1, 0) + countMatches(comparisonExample0.items, 1, 0)) / (countMatches(comparisonExample1.items, 1, 0) + countMatches(comparisonExample0.items, 1, 0) + countMatches(comparisonExample1.items, 1, 1) + countMatches(comparisonExample0.items, 1, 1))).toFixed(3) );
...
function makeBetaItems(category, n, a, b) {
  var items = [];

  	for (var item = 0; item < n; item++){

  		rand = jStat.beta.sample(a,b);
  		
  		items.push(new Item(category, Math.random() <  rand, Math.floor(100*rand)));    
  	}
...
function Optimizer(model0, model1, stepSize) {
  ...
  extent0[1] += stepSize;
  extent1[1] += stepSize;
  extentAll = [Math.min(extent0[0],extent1[0]), Math.max(extent0[1],extent1[1])];
  ...
  function maximizeWithConstraint(constraint, event) {
    ...
    var bestT0;
    var bestT1;
    for (var t0 = extentAll[0]; t0 <= extentAll[1]; t0 += stepSize) {
      for (var t1 = extentAll[0]; t1 <= extentAll[1]; t1 += stepSize) {
